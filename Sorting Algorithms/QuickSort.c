...
int partition_rpiv(int* arr, int left, int right) 
{
	int pivot = arr[right];
	int i = left - 1, j;
...
void quick_sort_rpiv(int* arr, int left, int right) 
{
	if (left < right)
	{
		//print_arr(right - left + 1, arr + left);
		int barr = partition_rpiv(arr, left, right);
		quick_sort_rpiv(arr, left, barr - 1);
		quick_sort_rpiv(arr, barr + 1, right);
	}
	
}

#define QUICKSORT(size, arr) quick_sort_rpiv(arr, 0, size - 1)
